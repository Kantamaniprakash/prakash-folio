...
export class LabArea extends Area
{
    ...
    setImages()
    {
        ...
        {
            const path = `lab/images/${import.meta.env.VITE_COMPRESSED ? key : key.replace(/\.ktx$/, '.png')}`
            ...
            if(!resource)
            {
                ...
                loader.load(
                    ...
                    (loadedTexture) =>
                    {
                        if(!import.meta.env.VITE_COMPRESSED) { loadedTexture.flipY = false; loadedTexture.needsUpdate = true }
        ...
        // Minis
        {
            ...
            for(const project of labData)
            {
                ...
                const loader = this.game.resourcesLoader.getLoader(compressed ? 'textureKtx' : 'texture')
                        ...
                        loader.load(
                            `lab/images/${import.meta.env.VITE_COMPRESSED ? project.imageMini : project.imageMini.replace(/\.ktx$/, '.png')}`,
                            (loadedTexture) =>
                            {
